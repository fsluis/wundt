...
public class JTimeLogger {
    ...
    public JTimeLogger(Log log, String name) {
        this.log = log;
        this.name = name;
    ...
    public JTimeLogger(Log log, String name, int maxN) {
        this.maxN = maxN;
        this.log = log;
        this.name = name;
        stats = new SummaryStatistics();
    ...
    public void reset() {
        log.info("Time statistics "+name+": N="+stats.getN()+"; mean="+stats.getMean()+"; std="+stats.getStandardDeviation());
        stats.clear();
    }
    ...
    private static List<JTimeLogger> loggers = new Vector<JTimeLogger>();
    public static void init(final int seconds) {
        GO = true;
        new Thread(new Runnable() {
            @Override
            public void run() {
                while(JTimeLogger.GO) {
                    try {
                        Thread.currentThread().sleep(1000*seconds);
                    } catch (InterruptedException e) {
                    }
                    for(JTimeLogger logger : loggers)
